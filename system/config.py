...
class SoVITSConfig(BaseModel):
    ...
    enabled: bool = Field(default=False, description="启用 SoVITS（优先级高，为 true 时无视其他 TTS 配置）")
    api_url: str = Field(default="http://127.0.0.1:9880", description="SoVITS API 地址")
    reference_audio: str = Field(default="", description="参考音频路径")
    reference_text: str = Field(default="", description="参考音频文本")
    language: str = Field(default="zh", description="语言代码 (zh/ja/en)")
...
class TTSConfig(BaseModel):
    ...
    sovits: SoVITSConfig = Field(default_factory=SoVITSConfig)
