# app_launcher_agent.py - 通用应用启动器Agent
import os
import json
import asyncio
import logging
...
class AppLauncherAgent:
    ...
    def __init__(self, config: Dict = None):
        ...
        self.logger.info("App Launcher Agent 初始化完成")
...
# 工厂函数
def create_app_launcher_agent(config: Dict = None) -> AppLauncherAgent:
    """创建Agent实例"""
    return AppLauncherAgent(config)
...
def get_agent_metadata() -> Dict:
    return {
        ...
        "agentType": "mcp",
        "entryPoint": {
            "module": "mcpserver.agent_open_launcher.app_launcher_agent",
            "class": "AppLauncherAgent"
        },
        "factory": {
            ...
            "invocationCommands": [
                {
                    "command": "启动应用",
                    "description": "启动应用程序",
                    "example": "{\"tool_name\":\"启动应用\",\"app\":\"记事本\"}"
                },
                {
                    "command": "获取应用列表",
                    "description": "获取可用应用列表",
                    "example": "{\"tool_name\":\"获取应用列表\"}"
                }
            ]
        }
    }
...
async def test_agent():
    ...
    agent = create_app_launcher_agent()
