...
async def _generate_audio(text, voice, response_format, speed):
    ...
    try:
        ...
        raise RuntimeError(
            f"FFmpeg error during audio conversion: {e}"
        )  # The raised error will still have details via e
...
def generate_speech_edgetts(text, voice, response_format, speed=1.0):
...
def generate_speech(text, voice, response_format, speed=1.0):
    ...
    优先级：sovits.enabled > edge_tts
    """
    if hasattr(config.tts, "sovits") and config.tts.sovits.enabled:
        from voice.output.sovits_handler import generate_speech_sovits

        return generate_speech_sovits(text)
    return generate_speech_edgetts(text, voice, response_format, speed)
